...
int main()
{
    ...
    /////////////////////////////////////////////
        ...
        /////////////////////////////////            
            int *host_buffer_1=(int *)malloc(vec_size_bytes);
            int *host_buffer_2=(int *)malloc(vec_size_bytes);
            int *host_buffer_3=(int *)malloc(vec_size_bytes);
            
            cl_mem device_buffer_1=clCreateBuffer(context, CL_MEM_WRITE_ONLY, vec_size_bytes, NULL, &cl_error);
            if(cl_error!=CL_SUCCESS)
            {
                printf("\nERROR: clCreateBuffer device_buffer_1; ERROR code:%i", cl_error);
                printf("\n\n");
                return 0;
            }
            
            cl_mem device_buffer_2=clCreateBuffer(context, CL_MEM_WRITE_ONLY, vec_size_bytes, NULL, &cl_error);
            if(cl_error!=CL_SUCCESS)
            {
                printf("\nERROR: clCreateBuffer device_buffer_2; ERROR code:%i", cl_error);
                printf("\n\n");
                return 0;
            }
            
            cl_mem device_buffer_3=clCreateBuffer(context, CL_MEM_WRITE_ONLY, vec_size_bytes, NULL, &cl_error);
            if(cl_error!=CL_SUCCESS)
            {
                printf("\nERROR: clCreateBuffer device_buffer_3; ERROR code:%i", cl_error);
                printf("\n\n");
                return 0;
            }
        ...
        /////////////////////////////////////////////////////
            cl_error=clSetKernelArg(kernel, 0, sizeof(cl_mem), &device_buffer_1);
            if(cl_error!=CL_SUCCESS)
            {
                printf("\nERROR: clSetKernelArg device_buffer_1; ERROR code:%i", cl_error);
                printf("\n\n");
                return 0;
            }
            
            cl_error=clSetKernelArg(kernel, 0, sizeof(cl_mem), &device_buffer_2);
            if(cl_error!=CL_SUCCESS)
            {
                printf("\nERROR: clSetKernelArg device_buffer_2; ERROR code:%i", cl_error);
                printf("\n\n");
                return 0;
            }
            
            cl_error=clSetKernelArg(kernel, 0, sizeof(cl_mem), &device_buffer_3);
            if(cl_error!=CL_SUCCESS)
            {
                printf("\nERROR: clSetKernelArg device_buffer_3; ERROR code:%i", cl_error);
                printf("\n\n");
                return 0;
            }
	...
	///////////////////////////
        ...
        for( int i=0; i< ( vec_size_bytes/sizeof(int) ); i++ )
        {
            host_buffer_1[i] = i*5;
        }
        
         cl_error=clEnqueueWriteBuffer(command_queue, device_buffer_1, CL_TRUE, 0, vec_size_bytes, host_buffer_1, 0, NULL, NULL);
        if(cl_error!=CL_SUCCESS)
        {
        ...
        for( int i=0; i< ( vec_size_bytes/sizeof(int) ); i++ )
        {
         ...
         cl_error=clEnqueueWriteBuffer(command_queue, device_buffer_2, CL_TRUE, 0, vec_size_bytes, host_buffer_2, 0, NULL, NULL);
        if(cl_error!=CL_SUCCESS)
        {
            printf("\nERROR: clEnqueueReadBuffer; ERROR code:%i", cl_error);
            printf("\n\n");
        ...
        printf("host_buffer_1:\n\t");
        for( int i=0; i<N; i++ )
        {
            printf("%i\t", host_buffer_1[i] );
        }
        printf("\n");
        
        printf("host_buffer_2:\n\t");
        for( int i=0; i<N; i++ )
        {
            printf("%i\t", host_buffer_2[i] );
        }
        printf("\n");

        
        
        //printf("\tbufferParaEnviar[0]=%i | bufferParaEnviar[1]=%i | bufferParaEnviar[%i]=%i\n", bufferParaEnviar[0], bufferParaEnviar[1], N-1, bufferParaEnviar[N-1]);
	...
	/////////////////////////////////////////////////
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            size_t local_work_size=15;                                           // threadsPerBlock
            size_t global_work_size=ceil( (float)N / (float)local_work_size)*(float)local_work_size;    // DimensãoTotal
            
            printf("//////////////////////////////////////////////////////////////////////////\n");
            printf("//\t\t\t\tLaunch:\t\t\t\t\t//\n");
            printf("//////////////////////////////////////////////////////////////////////////\n");
            
            printf("\tWork dimension: %u | Global work size: %zu | Local work size: %zu | # of Work-groups: %i\n", work_dim, global_work_size, local_work_size, (int)(global_work_size/local_work_size));
            // 9.2. Launch kernel 
    ...
    /////////////////////////////////////////////////
        cl_error=clEnqueueReadBuffer(command_queue, device_buffer_3, CL_TRUE, 0, vec_size_bytes, host_buffer_3, 0, NULL, NULL);
        if(cl_error!=CL_SUCCESS)
        {
            printf("\nERROR: clEnqueueReadBuffer; ERROR code:%i", cl_error);
            printf("\n\n");
        ...
        printf("host_buffer_3:\n\t");
        for( int i=0; i<N; i++ )
        {
            printf("%i\t", host_buffer_3[i] );
        }
        printf("\n");
    ...
    /////////////////////////////////////////
        clReleaseMemObject(device_buffer_1);
        clReleaseMemObject(device_buffer_2);
        clReleaseMemObject(device_buffer_3);
